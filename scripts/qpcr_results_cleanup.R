...
# Input: an exported .csv from the Design and Analysis software (each row is a well)
# Output: an exported .csv with Cq mean, also calculates efficiencies. (each row is a sample)
# Make sure to check/modify in script: 1. file path, 2. sample ids of standards,
# 3. Concentrations of standards.
...
file <-
  "2020_TRES_Telomere_Labwork/qpcr_results_raw/2020-12-04_telo.csv"
run <- read.csv(file)
...
# CLEAN UP
run$Cq <-
  run$Cq %>% as.character() %>% as.numeric #Change "undetermined" to NAs, this will create a warning message
...
for (i in 1:nrow(results)) {
  ...
  standard_dev <-
    sd(test$Cq, na.rm = T) #what is the starting standard dev of the triplicate
  standard_dev <-
    ifelse(is.na(standard_dev), 1, standard_dev) #NAs cause errors; change to 1
  if (standard_dev <= 0.25) {
    #if sd < 0.25 calculate mean and sd on all 3
    ...
    #find a bad triplicate
    sd_matrix  <-
      sqrt(((dist(test$Cq) / 2) ^ 2) * 2) # manually calculate sd in distance matrix form
    if (all(is.na(sd_matrix)) == TRUE) {
      # if all triplicates failed, return NA
      good_samples <- NA
    }
    else {
      best_pair <-
        which(sd_matrix == min (sd_matrix, na.rm = T)) * # find the pair with the lowest sd
        as.numeric(min (sd_matrix, na.rm = T) < 0.25) # make sure that the lowest sd is within 0.25, return 0 if not.

      good_samples <-
        case_when(
          best_pair == 1 ~ test$Cq[-3],
          # exclude c
          best_pair == 2 ~ test$Cq[-2],
          # exclude b
          best_pair == 3 ~ test$Cq[-1],
          # exclude a
          best_pair == 0 ~ as.numeric(NA)
        ) #all are bad
    }
    results$Cq[i] <- mean(good_samples)
    results$Cq_sd[i] <- sd(good_samples)
    results$nsamps[i] <-
      case_when (all(is.na(good_samples)) == TRUE ~ as.numeric(NA),
...
slope <-
  lm(cqs ~ log10(concentrations))$coefficients[2] %>% as.numeric()
efficiency <-  (10 ^ (-1 / slope)) - 1
efficiency

# 2. Using all standard raw values
all_cqs <-
  filter(run, Sample %in% std1:std5) %>% arrange(Sample) %>% select(Cq) %>% pull()
all_con <- rep(concentrations, 3) %>% sort(decreasing = T)
slope2 <-
  lm(all_cqs ~ log10(all_con))$coefficients[2] %>% as.numeric()
efficiency2 <-  (10 ^ (-1 / slope2)) - 1
efficiency2
...
sum(is.na(results$Cq[1:121])) # how many samples failed QC
dplyr::filter(run, Sample == h20) %>% select(Cq) #inspect water, should be NA

#double check date and primer are right
date
primer
...
write.csv(
  results,
  paste(
    "2020_TRES_Telomere_Labwork/processed_results/",
    date,
    "_",
    primer,
    "_processed.csv",
